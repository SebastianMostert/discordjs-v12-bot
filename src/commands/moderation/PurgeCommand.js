...
module.exports = class PurgeCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!message.guild.me.permissionsIn(message.channel).has("MANAGE_MESSAGES")) return message.channel.send("I do not have \`MANAGE_MESSAGES\` permission.");
    ...
    const fetched = await message.channel.messages.fetch({
      ...
      try {
        ...
        console.log(err);
        message.channel.send("I was unable to delete the amount stated. Make sure the messages arent older then 14 days.")
      }
  }
}